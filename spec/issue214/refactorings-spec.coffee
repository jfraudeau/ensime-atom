...
describe 'applyParsedPatchesInEditors', ->
  ...
  fit 'should not fail https://github.com/ensime/ensime-atom/issues/214', ->
    fileName = temp.path({suffix: '.scala'})
    
    fileContents = """import scala.collection.immutable.HashMap
...
"""
    ...
    expectedResult = """import scala.collection.immutable.HashMap
import scala.concurrent.ExecutionContext.Implicits.global
import scala.concurrent.Future
import scala.util.Try
...
"""
    ...
    waitsForPromise ->
      atom.workspace.open(fileName).then (editor) ->
        expect(editor.getText()).toEqual(fileContents)
