...
const log = require('loglevel').getLogger('ensime');
...
const pixelPositionFromMouseEvent = function(editor, event) {
  ...
  const linesClientRect = elem.querySelectorAll(".lines")[0].getBoundingClientRect();
...
const screenPositionFromMouseEvent = (editor, event) => atom.views.getView(editor).component.screenPositionForMouseEvent(event);
  // This was broken:
  //editor.screenPositionForPixelPosition(pixelPositionFromMouseEvent(editor, event))

// from haskell-ide
const bufferPositionFromMouseEvent = (editor, event) => editor.bufferPositionForScreenPosition((screenPositionFromMouseEvent(editor, event)));
...
const mkClasspathFileName = (scalaVersion, ensimeServerVersion) => path.join(packageDir(), `classpath_${scalaVersion}_${ensimeServerVersion}`);

var packageDir = () => __guard__(atom.packages.getActivePackage('Ensime'), x => x.path) || atom.packages.resolvePackagePath('Ensime');
