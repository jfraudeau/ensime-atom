...
const log = require('loglevel').getLogger('documentation')
...
class Documentation {
  ...
  getPoint () {
    ...
    if (hasSelectedText) { return this.selectedPoint() } else { return this.cursorPoint() }
  }

  static formUrl (host, port, path) {
    const alreadyUrl = (path.indexOf('//') !== -1)
    if (alreadyUrl) { return path } else { return `http://${host}:${port}/${path}` }
  }
  ...
  static openDoc (url) {
    ...
    switch (split) {
      case 'external-browser':
        if (!this.shell) { this.shell = require('shell') }
        return this.shell.openExternal(url)
      default: return atom.workspace.open(url, {split})
    }
  }
}
...
const goToDocAtPoint = function (instance, editor) {
  if (instance) {
    ...
    return instance.api.getDocUriAtPoint(editor.getBuffer().getPath(), point).then(function (msg) {
      switch (msg.typehint) {
        case 'FalseResponse': return atom.notifications.addError('No documentation found')
        default: return Documentation.openDoc(Documentation.formUrl('localhost', instance.httpPort, msg.text))
      }
    })
  }
}

const goToDocIndex = instance => Documentation.openDoc(`http://localhost:${instance.httpPort}/docs`)
